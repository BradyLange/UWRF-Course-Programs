...
//Beginning of if and else statement
		if (guess < 0 || guess > 100) { //if the users input isn't what is asked for it will receive the message below
			JOptionPane.showMessageDialog(null, "Invalid input. Please try again.");
		counter ++; //adding 1 to the counter to keep track of how many times the user has guessed
			}	
		else
			if (guess == num && diff == 0) { //using the or statement so the user doens't input a number less than 0 or more than 100 on accident
